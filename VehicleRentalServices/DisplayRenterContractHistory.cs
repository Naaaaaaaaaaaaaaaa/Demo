...
{
    public partial class DisplayRenterContractHistory : Form
    {
        private Renter renter = new Renter();
        public DisplayRenterContractHistory()
        {
            InitializeComponent();
            LoadForm();
        ...
        private void ClickChooseToFeedback(object sender, EventArgs e)
        {
            if (CheckListContract())
            {
                ...
                {
                    var list = new List<Person> { Database.owner };
                    if (contract.GetDriver.FullName != "") list.Add(contract.GetDriver);
                    if (contract.GetAssistant.FullName != "") list.Add(contract.GetAssistant);
                    DisplayRenterChooseToFeedback display = new DisplayRenterChooseToFeedback(contract.GetRenter, list);
                    display.ShowDialog();
